    ...
    >
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        {stats.map((s) => (
          <div
            key={s.label}
            className="rounded-xl border border-slate-100 bg-slate-50/40 px-2 py-2.5 sm:px-3 sm:py-3"
          >
            <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">
              {s.label}
            </p>
            <p
              className={`mt-1 truncate font-mono text-sm font-bold tabular-nums sm:text-base ${s.accent}`}
            >
              {s.value}
            </p>
